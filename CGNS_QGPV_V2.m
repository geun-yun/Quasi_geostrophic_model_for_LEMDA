...
for m = 1:(N_t/100) - 1
    ...
    q_1_estimated_smoothed(:,:,m+1) = q_1_smoothed(:,:,m+1);
    q_2_estimated_smoothed(:,:,m+1) = q_2_smoothed(:,:,m+1);
end
toc

q_1_sum_cov = zeros(N_x, N_y);
q_2_sum_cov = zeros(N_x, N_y);
psi_1_sum_cov = zeros(N_x, N_y);
psi_2_sum_cov = zeros(N_x, N_y);
for m = 1:N_t/100
    q_1_sum_cov = q_1_sum_cov + q_1(:,:,m);
    q_2_sum_cov = q_2_sum_cov + q_2(:,:,m);
    psi_1_sum_cov = psi_1_sum_cov + psi_1(:,:,m);
    psi_2_sum_cov = psi_2_sum_cov + psi_2(:,:,m);
end
q_1_mean_cov = (100/N_t) * q_1_sum_cov;
q_2_mean_cov = (100/N_t) * q_2_sum_cov;
...
R_f2 = zeros(N_x, N_y);
for m = 1:N_t/100
    R_f1 = R_f1 + (q_1(:,:,m) - q_1_mean_cov) .* (psi_1(:,:,m) - psi_1_mean_cov);
    R_f2 = R_f2 + (q_2(:,:,m) - q_2_mean_cov) .* (psi_2(:,:,m) - psi_2_mean_cov);
end
R_f1 = (100/N_t) * R_f1;
R_f2 = (100/N_t) * R_f2;
toc
...
RMSE_2 = zeros(N_x, N_y);
mean_1 = zeros(N_x, N_y);
mean_2 = zeros(N_x, N_y);
for m = N_t/100:N_t - 1
    dw1 = (dt) * rand(N_x,N_y);
    dw2 = (dt) * rand(N_x,N_y);
    ...
    % Apply CGNS to get the estimation
    % Compute the CGNS matrices
    A_0_1 = ((-(2/kd2)*del2(psi_2(:,:,m+1), h_x, h_y) + psi_2(:,:,m+1)) ...
        - (-(2/kd2)*del2(psi_2(:,:,m), h_x, h_y) + psi_2(:,:,m))) / dt;
    A_0_2 = ((-(2/kd2)*del2(psi_1(:,:,m+1), h_x, h_y) + psi_1(:,:,m+1)) ...
        - (-(2/kd2)*del2(psi_1(:,:,m), h_x, h_y) + psi_1(:,:,m))) / dt;

    a_0_1 = 0; a_0_2 = 0;
    
    [a_1_1] = construct_a1(N_x,N_y,h_x,h_y,psi_1(:,:,m));
    [a_1_2] = construct_a1(N_x,N_y,h_x,h_y,psi_2(:,:,m));
    A_1_1 = (2/kd2) * a_1_2;
    A_1_2 = (2/kd2) * a_1_1;
    ...
    RMSE_1 = RMSE_1 + (q_1_estimated_smoothed(:,:,m+1) - q_1_smoothed(:,:,m+1)).^2;
    RMSE_2 = RMSE_2 + (q_2_estimated_smoothed(:,:,m+1) - q_2_smoothed(:,:,m+1)).^2;
    mean_1 = mean_1 + q_1_smoothed(:,:,m+1);
    mean_2 = mean_2 + q_2_smoothed(:,:,m+1);
end
toc

%% Visualization
mean_1 = mean_1 / (99*N_t/100);
mean_2 = mean_2 / (99*N_t/100);
std_1 = zeros(N_x,N_y);
std_2 = zeros(N_x,N_y);
for m = (N_t/100) + 1:N_t
    std_1 = std_1 + (mean_1 - q_1_smoothed(:,:,m)).^2;
    std_2 = std_2 + (mean_2 - q_2_smoothed(:,:,m)).^2;
end
std_1 = sqrt(std_1 / (99*N_t/100));
std_2 = sqrt(std_2 / (99*N_t/100));
RMSE_1 = sqrt(RMSE_1 / (99*N_t/100));
RMSE_2 = sqrt(RMSE_2 / (99*N_t/100));
figure; imagesc(RMSE_1);
figure; imagesc(RMSE_2);
...
videoFileName = ['sin_CGNS2_smoothed_proper_cov',num2str(smooth_size),'_Simulation_Nt', num2str(N_t), 'T', num2str(T), '_Nx', num2str(N_x),'.mp4'];
...
%%
function [a1] = construct_a1(N_x,N_y,h_x,h_y,psi)
    % Total number of grid points
    N = N_x * N_y;
